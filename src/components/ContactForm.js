...
const ContactForm = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData, // Ensure the rest of formData stays intact
      [name]: value,
    });
  };
        ...
        <div className="mb-4 flex space-x-4">
          <div className="lg:w-1/2">
            ...
            {errors.name  && <motion.p
             initial={{ opacity:0}}
             whileInView={{opacity:1}}
            className="text-rose-800">{errors.name}</motion.p>}
          </div>
          <div className="lg:w-1/2">
            <input
              type="email"
              id="email"
              name="email"
              value={formData.email} // Corrected to bind email value
              placeholder="Email"
              onChange={handleChange}
              className="mb-8 w-full appearance-none rounded-lg border border-stone-50/30 bg-transparent px-3 py-2 text-sm focus:border-stone-400 focus:outline-none"
            />
            ...
            initial={{ opacity:0}}
            whileInView={{opacity:1}}
            className="text-rose-800">{errors.email}</motion.p>}
          </div>
        </div>
        <div className="mb-4">
          <textarea
          ...
          initial={{ opacity:0}}
          whileInView={{opacity:1}}
          className="text-rose-800">{errors.message}</motion.p>}
        </div>
        <button
          type="submit"
